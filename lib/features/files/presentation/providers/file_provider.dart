...
final foldersProvider = FutureProvider.autoDispose<List<WorkspaceFolder>>((ref) async {
  final repo = ref.watch(fileRepositoryProvider);
...
class UploadProgressNotifier extends Notifier<UploadProgressState> {
  ...
  Future<void> uploadFile(String filePath, String fileName) async {
    ...
    state = state.copyWith(isUploading: true, progress: 0.0, fileName: fileName, error: null, result: null);
    try {
...
final uploadProgressProvider = NotifierProvider<UploadProgressNotifier, UploadProgressState>(
