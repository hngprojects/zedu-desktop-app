  ...
  group('UserProfileNotifier Tests', () {
    ...
    setUp(() {
      ...
      workspaceState = WorkspaceState(
        ...
        selectedWorkspace: const Workspace(id: 'org-1', name: 'Org 1', avatar: ''),
      );

      // Default stubs for load()
      registerFallbackValue(const ProfileAccount(name: '', email: '', timezone: ''));
      ...
      when(() => mockRepository.getAccount()).thenAnswer(
        (_) async => const Success(ProfileAccount(name: 'User One', email: 'test@example.com', timezone: 'UTC')),
      );
      when(() => mockRepository.getNotificationPreferences()).thenAnswer(
        (_) async => const Success(NotificationPreferences(
          mode: NotificationMode.allMessages,
          fromTime: '',
          toTime: '',
      ...
      when(() => mockRepository.getSecuritySessions()).thenAnswer(
        (_) async => const Success(<SecuritySession>[]),
      );
      when(() => mockRepository.getOrganization()).thenAnswer(
        (_) async => const Success(OrganizationProfile(id: 'org-1', name: 'Org 1', natureOfBusiness: '', country: '')),
      );
      when(() => mockRepository.getTeamMembers(orgId: any(named: 'orgId'))).thenAnswer(
        (invocation) async {
          final id = invocation.namedArguments[#orgId] as String?;
          if (id == 'org-1') {
            return const Success([
              TeamMember(id: 'm-1', email: 'm1@org1.com', role: 'User', dateJoined: '', status: TeamMemberStatus.active, name: 'Member 1'),
            ]);
          } else if (id == 'org-2') {
            return const Success([
              TeamMember(id: 'm-2', email: 'm2@org2.com', role: 'User', dateJoined: '', status: TeamMemberStatus.active, name: 'Member 2'),
            ]);
          }
          return const Success(<TeamMember>[]);
        },
      );
      when(() => mockRepository.getRolesAndPermissions()).thenAnswer(
        (_) async => const Success(<RolePermission>[]),
      );
    ...
    ProviderContainer createContainer({WorkspaceState? initialWorkspaceState}) {
      ...
      final cont = ProviderContainer(
        overrides: [
          ...
          workspaceProvider.overrideWith(() => FakeWorkspaceNotifier(actualWorkspaceState)),
        ],
      );
    ...
    test('UserProfileNotifier loads data initially for selected workspace', () async {
      container = createContainer();

      // Trigger build and wait for async work
    ...
    test('rebuilds and updates team members list when current workspace ID changes', () async {
      container = createContainer();
      ...
      // Verify first workspace loaded
      expect(container.read(userProfileNotifierProvider).teamMembers.first.id, 'm-1');

      // Switch workspace
      final notifier = container.read(workspaceProvider.notifier);
      await notifier.switchWorkspace(const Workspace(id: 'org-2', name: 'Org 2', avatar: ''));

      // Wait for async load triggered by notifier build
